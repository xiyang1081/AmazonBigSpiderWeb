...
func (this *BigController) Index() {
	...
	this.TplName = this.GetTemplate() + "/big/uklist.html"
...
func (this *BigController) Asin() {
	...
	DB := orm.NewOrm()
	err := DB.Using("ukhashdb")
	if err != nil {
		beego.Error("ukhashdb err:" + err.Error())
		this.Rsp(false, err.Error())
	}
	var maps []orm.Params
	...
	this.Data["title"] = "Amazon.uk:" + asin
	this.TplName = this.GetTemplate() + "/big/ukasin.html"

}
func (this *BigController) Query() {
	DB := orm.NewOrm()
	err := DB.Using("ukdatadb")
	if err != nil {
		beego.Error("ukdatadb err:" + err.Error())
		this.Rsp(false, err.Error())
	}
	asin := this.GetString("asin")
...
func (this *BigController) Export() {
	...
	for _, i := range maps {
		...
		w.Write([]string{temp["title"], temp["id"], "https://www.amazon.co.uk/dp/" + temp["id"], temp["bigname"], temp["rank"], temp["price"], temp["reviews"], temp["score"], temp["ship"], temp["sold"], temp["img"], temp["createtime"]})
