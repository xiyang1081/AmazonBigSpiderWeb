...
func (this *BigController) Index() {
	...
	this.TplName = this.GetTemplate() + "/big/delist.html"
...
func (this *BigController) Asin() {
	...
	DB := orm.NewOrm()
	err := DB.Using("dehashdb")
	if err != nil {
		beego.Error("dehashdb err:" + err.Error())
		this.Rsp(false, err.Error())
	}
	var maps []orm.Params
	...
	this.Data["title"] = "Amazon.de:" + asin
	this.TplName = this.GetTemplate() + "/big/deasin.html"

}
func (this *BigController) Query() {
	DB := orm.NewOrm()
	err := DB.Using("dedatadb")
	if err != nil {
		beego.Error("dedatadb err:" + err.Error())
		this.Rsp(false, err.Error())
	}
	asin := this.GetString("asin")
...
func (this *BigController) Export() {
	...
	for _, i := range maps {
		...
		w.Write([]string{temp["title"], temp["id"], "https://www.amazon.de/dp/" + temp["id"], temp["bigname"], temp["rank"], temp["price"], temp["reviews"], temp["score"], temp["ship"], temp["sold"], temp["img"], temp["createtime"]})
