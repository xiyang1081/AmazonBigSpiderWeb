...
func (this *BigController) Index() {
	...
	this.TplName = this.GetTemplate() + "/big/jplist.html"
...
func (this *BigController) Asin() {
	...
	DB := orm.NewOrm()
	err := DB.Using("jphashdb")
	if err != nil {
		beego.Error("jphashdb err:" + err.Error())
		this.Rsp(false, err.Error())
	}
	var maps []orm.Params
	...
	this.Data["title"] = "Amazon.co.jp:" + asin
	this.TplName = this.GetTemplate() + "/big/jpasin.html"

}
func (this *BigController) Query() {
	DB := orm.NewOrm()
	err := DB.Using("jpdatadb")
	if err != nil {
		beego.Error("jpdatadb err:" + err.Error())
		this.Rsp(false, err.Error())
	}
	asin := this.GetString("asin")
...
func (this *BigController) Export() {
	...
	for _, i := range maps {
		...
		w.Write([]string{temp["title"], temp["id"], "https://www.amazon.co.jp/dp/" + temp["id"], temp["bigname"], temp["rank"], temp["price"], temp["reviews"], temp["score"], temp["ship"], temp["sold"], temp["img"], temp["createtime"]})
